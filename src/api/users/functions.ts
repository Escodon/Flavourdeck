	...
	setPersistence(auth, browserLocalPersistence)
		.then(() => {
			...
			return signInWithEmailAndPassword(auth, email, password)
				...
				.catch((error) => {
					log(
						`User with email ${email} failed to authenticate. Error: ${error.message}`,
						"authUser"
					);
...
var authStateFires: number = 0;
var initialAuthState: User | null;
auth.onAuthStateChanged((user: User | null) => {
	...
	listenForUserFnArray.forEach((fn) => {
		fn(user);
	});
});
